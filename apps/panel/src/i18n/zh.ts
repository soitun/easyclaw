export default {
  ...
  tutorial: {
    ...
    chat: {
      ...
      toolsSelectorTitle: "工具",
      toolsSelectorBody: "点击打开工具选择器。你可以为当前会话启用或禁用特定工具（如网页搜索、文件访问等）。被禁用的工具 Agent 将无法使用。",
      resetBtnTitle: "重置会话",
    ...
    account: {
      ...
      welcomeBody: "账户页面展示你的个人资料和订阅状态。在这里可以查看你的计划、管理账户和退出登录。",
      profileCardTitle: "个人资料卡片",
      profileCardBody: "一览你的账户概况——显示头像、姓名、邮箱、当前计划和注册时间。这张卡片让你快速了解自己的账户身份信息。",
      identityTitle: "你的身份",
      identityBody: "你的头像（根据姓名首字母生成）、显示名称和邮箱地址。邮箱是你注册或登录 EasyClaw 时使用的地址。",
      logoutTitle: "退出登录",
      logoutBody: "点击此按钮退出当前账户。你将被重定向到首页。需要身份验证的功能（如应用和浏览器配置）在重新登录前将无法访问。",
      planTitle: "当前计划",
      planBody: "显示你所在的订阅计划（如免费版、专业版、企业版）。你的计划决定了可用的功能和使用限制。",
      memberSinceTitle: "注册时间",
      memberSinceBody: "你的账户创建日期。帮助你了解使用 EasyClaw 的时长。",
      subscriptionTitle: "订阅状态",
      subscriptionBody: "关于你活跃订阅的详细信息。此部分仅在你拥有活跃订阅时显示，包含计划详情、支付状态、席位使用情况和到期日期。",
      subscriptionDetailsTitle: "订阅详情",
      subscriptionDetailsBody: "四项关键信息：计划名称、订阅状态（活跃或已过期）、席位使用情况（已使用的分配席位数）以及订阅有效期截止日期。请关注到期日期以避免服务中断。",
    },
  },
} as const;
