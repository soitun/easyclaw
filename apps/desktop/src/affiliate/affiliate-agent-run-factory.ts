...
export function renderWorkItemProjection(workItem: GQL.AffiliateWorkItem): string {
  ...
  return [
    ...
    ...(sample ? [
      ...
      `- Shipment Status: ${sample.shipmentStatus ?? ""}`,
      `- Content Fulfillment Status: ${sample.contentFulfillmentStatus ?? ""}`,
      `- Platform Application Status: ${sample.platformApplicationStatus ?? ""}`,
