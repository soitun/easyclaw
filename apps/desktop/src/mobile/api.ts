...
const mobileGraphql: EndpointHandler = async (req, res, _url, _params, ctx: ApiContext) => {
...
const pairingCode: EndpointHandler = async (_req, res, _url, _params, ctx: ApiContext) => {
...
const installUrl: EndpointHandler = async (_req, res, _url, _params, ctx: ApiContext) => {
  if (!ctx.mobileManager) {
    sendJson(res, 500, { error: "Mobile Manager not initialized" });
    return;
  }
...
const status: EndpointHandler = async (_req, res, _url, _params, ctx: ApiContext) => {
  if (!ctx.mobileManager) {
    sendJson(res, 500, { error: "Mobile Manager not initialized" });
    return;
  }
...
const deviceStatus: EndpointHandler = async (_req, res, _url, _params, ctx: ApiContext) => {
...
const disconnect: EndpointHandler = async (_req, res, url, _params, ctx: ApiContext) => {
  if (!ctx.mobileManager) {
    sendJson(res, 500, { error: "Mobile Manager not initialized" });
    return;
  }
